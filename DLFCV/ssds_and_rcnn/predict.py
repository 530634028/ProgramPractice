...
ap.add_argument("-m", "--model", required=True,
                help="base path for frozen checkpoint detection directory")
...
ap.add_argument("-c", "--min-confidence", type=int, default=0.5,
                help="minimum probability used to filter weak detection ")
...
with model.as_default():
    with tf.Session(graph=model) as sess:
        ...
        # for each bounding box we would like to konw the score
        ...
        for (box, score, label) in zip(boxes, scores, labels):
            ...
            cv2.putText(output, label, (startX, y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.3, COLORS[idx],
                        1)
