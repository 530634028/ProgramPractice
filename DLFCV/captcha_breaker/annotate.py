#
# Annotating our dataset which is downloaded from URL
# a: zhonghy
# date: 2018-7-31
#
#
...
for (i, imagePath) in enumerate(imagePaths):
    ...
    try:
        ...
        # find contours in the images, keeping only the four largest
        # ones
        cnts = cv2.findContours(thresh.copy(), cv2.RETR_EXTERNAL,
                               cv2.CHAIN_APPROX_SIMPLE)
        cnts = cnts[0] if imutils.is_cv2() else cnts[1]
        cnts = sorted(cnts, key=cv2.contourArea, reverse=True)[:4]
        ####### attention key wrong write to kery #######
        ...
        for c in cnts:
            ...
            # fi the ''' key is pressed, then ignore the character
            if key == ord("'"):
