"""
#
# Implement of ResNet using mxnet library
# a : zhonghy
# date: 2018-12-4
#
"""

"""we can use stage to set the model be ResNet18 ResNet50 ResNet118 and so on"""

# import the necessary packages
import mxnet as mx

class MxResNet:
    # use "bottleneck" module with pre-activation (He at al. 2016)
    @staticmethod
    def residual_module(data, K, stride, red=False, bnEps=2e-5,
        ...
        conv3 = mx.sym.Convolution(data=act3, pad=(0, 0),
                                   kernel=(1, 1), stride=(1, 1), num_filter=K,
                                   no_bias=True)
    ...
    def build(classes, stages, filters, bnEps=2e-5, bnMom=0.9):
        ...
        for i in range(0, len(stages)):
            # intialize the stride, then apply a residual module
            # used to reduce the spatial size of the input volume
            stride = (1, 1) if i == 0 else (2, 2)
            body = MxResNet50.residual_module(body, filters[i + 1],
